<script lang="ts">
    ...
    const handleToggle = (collectionName: string) => {
        const isInCollection = isBookInCollection(
            collections.find((c) => c.name === collectionName)!,
        );
        collectionsStore.toggleBookInCollection(
            collectionName,
            bookUrl,
            (message, type) => {
                if (type === "success") {
    ...
    onMount(() => {
        if (
            !collections.some((collection) =>
                collection.books.includes(bookUrl),
            )
        ) {
...
<Modal bind:open outsideclose>
    <div class="space-y-2 py-4">
        {#each collections as collection (collection.name)}
            <div
                class="flex flex-row items-center justify-between bg-slate-700 py-3 px-4 rounded"
            >
                ...
                <Button
                    on:click={() => handleToggle(collection.name)}
                    outline={!isBookInCollection(collection)}
        ...
        <div class="flex flex-row items-center gap-2">
            ...
            <p class="text-sm">
                Collections are saved locally in your browser. Prices
                automatically update when they are added to a collection.
            </p>
        </div>
    </div>
</Modal>
